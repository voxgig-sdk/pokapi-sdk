...
func TestPokemonDirect(t *testing.T) {
	...
	t.Run("direct-load-pokemon", func(t *testing.T) {
		setup := pokemonDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-pokemon", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func pokemonDirectSetup(mockres any) *pokemonDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["POKAPI_TEST_POKEMON_ENTID"]; ok {
