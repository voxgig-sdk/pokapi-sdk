...
// Pokapi SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class PokapiMakeError
{
    public static function call(?PokapiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PokapiContext([], null);
        }
        $op = $ctx->op ?? new PokapiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PokapiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PokapiError) ? $err->msg : (string)$err;
        $msg = "PokapiSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new PokapiError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof PokapiError) {
