...
class AbilityEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Ability(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = ability_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "ability." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POKAPI_TEST_ABILITY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $ability_ref01_data = null;
        if (count($ability_ref01_data_raw) > 0) {
            $ability_ref01_data = Helpers::to_map($ability_ref01_data_raw[0][1]);
        }

        // LOAD
        $ability_ref01_ent = $client->Ability(null);
...
function ability_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/ability/AbilityTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["ability01", "ability02", "ability03"] as $k) {
    ...
    $entid_env_raw = getenv("POKAPI_TEST_ABILITY_ENTID");
