...
class AbilityDirectTest < Minitest::Test
  def test_direct_load_ability
    setup = ability_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-ability", setup[:live] ? "live" : "unit")
    if _should_skip
...
def ability_direct_setup(mockres)
