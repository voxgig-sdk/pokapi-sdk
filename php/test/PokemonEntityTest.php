...
class PokemonEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = PokapiSDK::test($seed, null);
        $seen = iterator_to_array($base->Pokemon(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = PokapiConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = PokapiSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POKAPI_TEST_POKEMON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function pokemon_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("POKAPI_TEST_POKEMON_ENTID");
