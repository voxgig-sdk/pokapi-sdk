...
class PokemonSpeciesEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = PokapiSDK::test(null, null);
        $ent = $testsdk->PokemonSpecies(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = pokemon_species_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "pokemon_species." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POKAPI_TEST_POKEMON_SPECIES_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $pokemon_species_ref01_data = null;
        if (count($pokemon_species_ref01_data_raw) > 0) {
            $pokemon_species_ref01_data = Helpers::to_map($pokemon_species_ref01_data_raw[0][1]);
        }

        // LOAD
        $pokemon_species_ref01_ent = $client->PokemonSpecies(null);
...
function pokemon_species_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/pokemon_species/PokemonSpeciesTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["pokemon_species01", "pokemon_species02", "pokemon_species03"] as $k) {
    ...
    $entid_env_raw = getenv("POKAPI_TEST_POKEMON_SPECIES_ENTID");
