...
//     (\`new PokapiSDK(...)\` / \`PokapiSDK.test(...)\` become
//     \`PokapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"ability":{"test01":{"id":"test01"}},"paginated_resource_list":{"test01":{"id":"test01"}},"pokemon":{"test01":{"id":"test01"}},"pokemon_species":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PokapiSDK(...)\` and any
// \`PokapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PokapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PokapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
