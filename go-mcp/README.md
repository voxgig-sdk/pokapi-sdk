...
[MCP](https://modelcontextprotocol.io) server exposing the Pokapi SDK as
two agent tools — `pokapi_list` and `pokapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokapi-mcp)
make build

# 2. Provide credentials via the environment
export POKAPI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pokapi \
  -- /absolute/path/to/pokapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pokapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pokapi_list: first page of records
{ "entity": "pokemon" }
{ "entity": "pokemon", "query": { } }

// pokapi_load: one record by id
{ "entity": "ability", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pokapi \
     -- "$PWD"/dist/*/pokapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pokapi_list` and `pokapi_load` tools now appear
   in new sessions. Ask the agent to *"list pokemon using pokapi"*
   and it calls `pokapi_list` with `{"entity":"pokemon"}`.
...
```sh
export POKAPI_APIKEY=sk_live_xxx            # API key
export POKAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pokapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pokapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POKAPI_APIKEY` | API key sent with every request. |
| `POKAPI_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

ability | paginated_resource_list | pokemon | pokemon_species | type

### Smoke test via HTTP (raw JSON-RPC)

```sh
./pokapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pokapi_load","arguments":{"entity":"ability","query":{"id":1}}}}'
```
