...
class AbilityEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = ability_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "ability." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POKAPI_TEST_ABILITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    ability_ref01_data = nil
    if ability_ref01_data_raw.length > 0
      ability_ref01_data = Helpers.to_map(ability_ref01_data_raw[0][1])
    end

    # LOAD
    ability_ref01_ent = client.Ability(nil)
...
def ability_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "ability", "AbilityTestData.json")
  ...
  entid_env_raw = ENV["POKAPI_TEST_ABILITY_ENTID"]
