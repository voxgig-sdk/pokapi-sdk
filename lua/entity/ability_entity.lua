...
local AbilityEntity = {}
AbilityEntity.__index = AbilityEntity


function AbilityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AbilityEntity)
  self._name = "ability"
  self._client = client
...
function AbilityEntity:get_name()
  return self._name
end


function AbilityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AbilityEntity.new(self._client, opts)
end


function AbilityEntity:data_set(args)
  if args ~= nil then
...
function AbilityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AbilityEntity:match_set(args)
  if args ~= nil then
...
function AbilityEntity:match_get()
...
function AbilityEntity:stream(action, args, callopts)
...
---@param reqmatch AbilityLoadMatch
---@param ctrl? table
---@return Ability
---@return string? err
function AbilityEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AbilityEntity:_run_op(ctx, post_done)
  local utility = self._utility
