...
class AbilityEntity
  ...
  def make
    opts = @_entopts.dup
    AbilityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PokapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Ability, Hash] the current Ability data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PokapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Ability fields)
  def match_get
  ...
  #
  # @param reqmatch [AbilityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Ability.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Ability, Hash] the loaded Ability; raises PokapiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PokapiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
