...
class FtCtrl extends PokapiControl
{
...
class FtClient
{
    ...
    private PokapiUtility $utility;

    public function __construct(PokapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): PokapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PokapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new PokapiUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (PokapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PokapiConfig::make_feature($name);
    ...
    private static function buildUrl(PokapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PokapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PokapiResponse($fetched);
        }
    ...
    private function fail(PokapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PokapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
