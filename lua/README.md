...
The Lua SDK for the Pokapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Ability(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Pokapi(nil):load(
  { id = "test01" }, nil
)
