...
class PokapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PokapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PokapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PokapiTestRunner.getenv("POKAPI_TEST_LIVE")
        override = PokapiTestRunner.getenv("POKAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PokapiTestRunner.getenv(key)
        ...
        explain = PokapiTestRunner.getenv("POKAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["POKAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PokapiTestRunner._test_control is not None:
            return PokapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PokapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PokapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PokapiTestRunner.load_test_control()
...
def load_env_local():
    PokapiTestRunner.load_env_local()


def env_override(m):
    return PokapiTestRunner.env_override(m)


def entity_list_to_data(lst):
    return PokapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PokapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PokapiTestRunner.load_test_control()


def live_delay_ms():
    return PokapiTestRunner.live_delay_ms()
