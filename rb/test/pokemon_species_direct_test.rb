...
class PokemonSpeciesDirectTest < Minitest::Test
  def test_direct_load_pokemon_species
    setup = pokemon_species_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-pokemon_species", setup[:live] ? "live" : "unit")
    if _should_skip
...
def pokemon_species_direct_setup(mockres)
