...
The Python SDK for the Pokapi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Ability(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Pokapi(None).load(
    {"id": "test01"}, None
)
