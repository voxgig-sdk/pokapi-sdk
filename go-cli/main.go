// Command Pokapi-cli is a CLI + REPL for the Pokapi SDK,
...
const entitiesHelp = "ability paginated_resource_list pokemon pokemon_species type"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      POKAPI_APIKEY (api key), POKAPI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: POKAPI_APIKEY carries the API key and
	// POKAPI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("POKAPI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("POKAPI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPokapiSDK(opts)
