...
// Pokapi SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class PokapiMakePoint
{
    public static function call(PokapiContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof PokapiError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = PokapiHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = PokapiHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
