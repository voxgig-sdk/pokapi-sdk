...
AQL-driven command-line client **and** interactive REPL for the Pokapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokapi-cli)
make build
...
./pokapi-cli load 1 ability            # {id:1} shorthand
./pokapi-cli load '{id:1}' ability       # explicit match map

# 4. Override the API base URL for a single call
POKAPI_BASE=https://api.example.com ./pokapi-cli load 1 ability

# 5. No arguments -> interactive REPL
./pokapi-cli
pokapi> load 1 ability
pokapi> :quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokapi-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pokapi-cli load 1 ability
   ```
...
```sh
./pokapi-cli load 1 ability          # scalar shorthand for {id:1}
./pokapi-cli load '{id:1}' ability     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `pokapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `ability`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pokapi-cli`. |
...
(`./pokapi-cli <expr>`) and interactively (the REPL), and expressions compose the
